 ...
 * 2013-03-24
 */
class mod_meal_menu_group extends inc_mod_meal {

	// 获取，移动分组列表
	function get_group_select() {
		$id  = (int)fun_get::get("id");
		$where = " group_id!='" . $id . "'";
		$arr = tab_meal_menu_group::get_list_layer( 0 , 1 , $where);
		$arr_select = array();
		//添加默认
	...
	function on_move_save() {
		...
		$arr = tab_meal_menu_group::on_move($id , $pid);
		if($arr['code']==0) {
	...
	function on_save_all() {
		...
		//首先删除没在保存id中的所有记录
		$arr = tab_meal_menu_group::on_delete(array(),$str_where);
		for( $i = 1 ; $i < $lng_count ; $i++) {
			$arr_fields = array(
			...
			$arr_resave[$arr_group_id_layer[$i]] = tab_meal_menu_group::on_save($arr_fields);
